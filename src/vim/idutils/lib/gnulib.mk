...
# Reproduce by: gnulib-tool --import --dir=. --local-dir=gl --lib=libgnu --source-base=lib --m4-base=m4 --doc-base=doc --tests-base=gnulib-tests --aux-dir=build-aux --with-tests --avoid=canonicalize-lgpl --avoid=lock --no-libtool --macro-prefix=gl alloca alloca-opt announce-gen argv-iter autobuild calloc closeout dirname error exclude exit exitfail extensions fflush fnmatch fnmatch-gnu fopen fpending fprintf-posix freopen getcwd getopt-gnu gettext git-version-gen gitlog-to-changelog gnu-make gnu-web-doc-update gnumakefile gnupload havelib ignore-value inttostr lstat maintainer-makefile malloc manywarnings mbchar mbuiter mempcpy obstack pathmax perl posix-shell printf-posix progname quote quotearg realloc regex sprintf-posix stat-macros stdbool strcase strcasestr strnlen1 strsep sys_ioctl update-copyright useless-if-before-free vc-list-files warnings xalloc xalloc-die xgetcwd
...
libgnu_a_SOURCES += argv-iter.c argv-iter.h
...
EXTRA_libgnu_a_SOURCES += fprintf.c
...
EXTRA_libgnu_a_SOURCES += freopen.c
...
EXTRA_libgnu_a_SOURCES += sprintf.c
